...
# `// []` before the iteration, not after: a repository with no topics answers
# `null` rather than `[]`, and `.repositoryTopics[]` then aborts the whole
# script with "cannot iterate over: null" — on the FIRST run, which is the
# only run a fresh repository ever has.
cur_topics=$(gh_field repositoryTopics '(.repositoryTopics // [])[].name')
